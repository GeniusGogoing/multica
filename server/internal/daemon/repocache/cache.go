...
import (
	...
	"net/url"
	"os"
	"os/exec" // needed for *exec.ExitError type assertion in readFetchRefspec
	"path/filepath"
	"regexp"
	"strconv"
	"strings"
...
func gitCloneBare(url, dest string) error {
	cmd := gitCommand("clone", "--bare", url, dest)
	cmd.Env = gitEnv()
...
func gitFetch(barePath string) error {
	...
	// on this call.
	cmd := gitCommand("-C", barePath, "remote", "set-head", "origin", "--auto")
	cmd.Env = gitEnv()
	_ = cmd.Run()
	return nil
}
...
func runGitFetch(barePath string) error {
	cmd := gitCommand("-C", barePath, "fetch", "origin")
	cmd.Env = gitEnv()
	if out, err := cmd.CombinedOutput(); err != nil {
...
func readFetchRefspec(barePath string) (string, error) {
	out, err := gitCommand("-C", barePath, "config", "--get", "remote.origin.fetch").Output()
	if err != nil {
...
func setFetchRefspec(barePath, refspec string) error {
	out, err := gitCommand("-C", barePath, "config", "remote.origin.fetch", refspec).CombinedOutput()
	if err != nil {
...
func gitRefExists(repoPath, ref string) bool {
	cmd := gitCommand("-C", repoPath, "rev-parse", "--verify", "--quiet", ref)
	return cmd.Run() == nil
}
...
func updateExistingWorktree(worktreePath, branchName, baseRef string) (string, error) {
	...
	resetCmd := gitCommand("-C", worktreePath, "reset", "--hard")
	...
	cleanCmd := gitCommand("-C", worktreePath, "clean", "-fd")
	...
	// startpoint.
	checkoutCmd := gitCommand("-C", worktreePath, "checkout", "-b", branchName, baseRef)
...
func getRemoteDefaultBranch(barePath string) string {
	...
	if out, err := gitCommand("-C", barePath, "symbolic-ref", "refs/remotes/origin/HEAD").Output(); err == nil {
		ref := strings.TrimSpace(string(out))
		if ref != "" {
			if err := gitCommand("-C", barePath, "rev-parse", "--verify", ref).Run(); err == nil {
				return ref
			}
		}
	}
	...
	for _, candidate := range []string{"refs/remotes/origin/main", "refs/remotes/origin/master"} {
		if err := gitCommand("-C", barePath, "rev-parse", "--verify", candidate).Run(); err == nil {
			return candidate
		}
	}
	...
	if bareRef != "" {
		...
		if err := gitCommand("-C", barePath, "rev-parse", "--verify", originRef).Run(); err == nil {
			return originRef
		}
	}
	...
	if out, err := gitCommand("-C", barePath, "for-each-ref", "--format=%(refname)", "refs/remotes/origin/").Output(); err == nil {
...
func bareHeadBranch(barePath string) string {
	out, err := gitCommand("-C", barePath, "symbolic-ref", "HEAD").Output()
	if err != nil {
		return ""
	}
	ref := strings.TrimSpace(string(out))
